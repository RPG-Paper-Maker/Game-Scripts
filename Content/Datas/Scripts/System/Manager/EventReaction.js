...
*/
import { Node, Portion } from "../Core/index.js";
import { Enum, Utils, Platform } from "../Common/index.js";
...
class EventReaction {
    ...
    static sendEvent(sender, targetKind, targetID, isSystem, eventID, parameters, senderNoReceiver) {
        switch (targetKind) {
            case 0: // Send to all
                Manager.EventReaction.sendEventDetection(sender, -1, isSystem, eventID, parameters);
                break;
            case 1: // Send to detection
                Manager.EventReaction.sendEventDetection(sender, targetID, isSystem, eventID, parameters, senderNoReceiver);
                break;
    ...
    static sendEventDetection(sender, targetID, isSystem, eventID, parameters, senderNoReceiver = false) {
        Manager.Stack.currentMap.updatePortions(this, function (x, y, z, i, j, k) {
            ...
            // Moved objects
            Manager.EventReaction.sendEventObjects(objects.min, sender, targetID, isSystem, eventID, parameters, senderNoReceiver);
            Manager.EventReaction.sendEventObjects(objects.mout, sender, targetID, isSystem, eventID, parameters, senderNoReceiver);
            // Static
            let mapPortion = Manager.Stack.currentMap.getMapPortion(new Portion(i, j, k));
            if (mapPortion) {
                Manager.EventReaction.sendEventObjects(mapPortion.objectsList, sender, targetID, isSystem, eventID, parameters, senderNoReceiver);
            }
        });
        // And the hero!
