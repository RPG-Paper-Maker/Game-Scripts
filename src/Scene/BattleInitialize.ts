...
class BattleInitialize {
    ...
    initializeAlliesBattlers() {
        ...
        let position: Vector3, player: Player, battler: Battler, center: Vector3, 
            offset: Vector3;
        for (let i = 0; i < l; i++) {
            // Battlers
            ...
            battler = new Battler(player, false, Position.createFromVector3(position),
    ...
    initializeEnemiesBattlers() {
        ...
        let troopMonster: System.TroopMonster, position: Vector3, player: Player,
            battler: Battler, center: Vector3, offset: Vector3;
        for (let i = 0; i < l; i++) {
            // Battlers
            ...
            battler = new Battler(player, true, Position.createFromVector3(position), 
                position, this.battle.camera);
            player.battler = battler;
    ...
    public initializeWindowCommands() {
        ...
        for (let i = 0; i < l; i++) {
            ...
            listContent[i] = Graphic.TextIcon.createFromSystem(skill.name(), 
                skill);
