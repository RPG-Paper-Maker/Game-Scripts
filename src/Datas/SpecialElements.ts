...
import { Constants, Enum, IO, Paths, Platform, Utils } from "../Common";
...
class SpecialElements {
     ...
     */
    static async updateTextureAutotile(textureAutotile: TextureBundle, texture: 
        THREE.Texture, id: number)
    {
     ...
     *  @returns {any[]}
     */
    static async loadTextureMountain(textureMountain: TextureBundle, texture: 
        THREE.Texture, picture: System.Picture, offset: number, id: number): 
        Promise<any[]>
    {
        let picture2D = await Picture2D.create(picture);
     ...
     *  @param {number} id - The picture ID
     */
    static async updateTextureMountain(textureMountain: TextureBundle, texture: 
        THREE.Texture, id: number)
    {
