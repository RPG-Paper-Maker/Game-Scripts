...
class WindowBox extends Bitmap
{
	 ...
	 *  of content
	 */
	draw(isChoice: boolean = false, windowDimension: number[] = this.windowDimension, contentDimension: number[] = this.contentDimension)
	{
		// Content behind
		if (this.content)
		{
			this.content.drawBehind(
				ScreenResolution.getScreenX(contentDimension[0]),
				ScreenResolution.getScreenY(contentDimension[1]),
				ScreenResolution.getScreenX(contentDimension[2]),
				ScreenResolution.getScreenY(contentDimension[3])
			);
		}
		...
		if (this.content)
		{
			if (!isChoice && this.limitContent)
			{
				...
				Platform.ctx.rect(
					ScreenResolution.getScreenX(contentDimension[0]),
					ScreenResolution.getScreenY(contentDimension[1] - this.padding[3] / 2),
					ScreenResolution.getScreenX(contentDimension[2]),
					ScreenResolution.getScreenY(contentDimension[3] + this.padding[3])
				);
				Platform.ctx.clip();
			}
			if (isChoice)
