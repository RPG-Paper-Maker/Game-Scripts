...
class MapPortion {
	...
	public terrainFloors: StructMapElementCollision[][];
	...
	public staticMountainsList: Map<number, Mountains>;
	 ...
	 *  @param {Record<string, any>} json - object describing the map portion
	 */
	async readStatic(json: Record<string, any>) {
	 ...
	 */
	async readSprites(json: Record<string, any>) {
	 ...
	 */
	updateCollisionSprite(collisions: StructMapElementCollision[], position: Position) {
		let i: number,
			l: number,
			a: number,
			b: number,
	...
	updateCollision(
		...
		let i: number,
			...
			c: number,
			positionPlus: Position,
			objCollisionPlus: StructMapElementCollision;
		for (i = 0, l = collisions.length; i < l; i++) {
			objCollision = collisions[i];
	...
	isPositionIn(position: Position): boolean {
		return (
			this.portion.x === Math.floor(position.x / Constants.PORTION_SIZE) &&
