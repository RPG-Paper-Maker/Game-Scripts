...
class MapObject {
	...
	public landCollision: StructMapElementCollision;
	public landCollisions: StructMapElementCollision[];
	public frame: Frame;
	...
	private objectAutotileBundle: TextureBundle | null = null;
	...
	private getObjectAutotileTileID(position: Position): number {
		...
		const sameAutotileAt = (x: number, z: number) => {
			for (const object of objects) {
				...
				if (
					...
					object.positionLayer + (other.layer.getValue() as number) ===
						this.positionLayer + (state.layer.getValue() as number) &&
	...
	static refreshAutotilesAround(position: THREE.Vector3) {
	...
	private refreshAutotile() {
		if (this.mesh === null || this.objectAutotileBundle === null) return;
		const state = this.currentStateInstance;
	...
	async changeState(dontChangeOrientation = false) {
		...
		if (this.currentStateInstance !== null) {
			if (this.currentStateInstance.graphicKind === ELEMENT_MAP_KIND.OBJECT_3D) {
				...
				this.objectAutotileBundle =
					bundles?.find((bundle) =>
						bundle.isInTexture(autotileData.pictureID, this.currentStateInstance.rectTileset),
					) ?? null;
		...
		if (this.currentState !== null && !this.isNone() && (texture || isGltfNoTexture)) {
			...
			positionTranformation.layer = this.positionLayer + (this.currentStateInstance.layer.getValue() as number);
			if (this.currentStateInstance.graphicKind === ELEMENT_MAP_KIND.OBJECT_3D) {
				...
				const collision = autotile.updateGeometryAutotile(
					result[0],
					bundle,
					positionTranformation,
					width,
					height,
					autotileData.pictureID,
					0,
				);
			...
			if (this.moving) {
				...
				if (this.currentStateInstance.moveAnimation) {
					frame = this.frame.update(
						(Data.Systems.mapFrameDuration.getValue() as number) / (this.speed.getValue() as number),
					);
				}
				...
				this.mesh.position.set(
					this.position.x,
					this.position.y + offset + this.getMapObjectLayerDepthOffset(),
					this.position.z,
				);
			} else {
	...
	private createObjectLight(settings: StateLight, group: THREE.Group) {
		...
		switch (kind) {
			...
			case 1: {
				...
				parent.rotation.set(
					THREE.MathUtils.degToRad(this.currentStateInstance.angleX.getValue() as number),
					this.getLightsAngleY((settings.followOrientation.getValue() as number) === 1),
					THREE.MathUtils.degToRad(this.currentStateInstance.angleZ.getValue() as number),
				);
				group.add(parent);
	...
	updateObjectLights() {
		...
		for (const { light, settings, target, parent } of this.objectLights) {
			...
			if (
				light instanceof THREE.SpotLight ||
				(light instanceof THREE.PointLight && (settings.followOrientation.getValue() as number) === 1)
			) {
				parent?.rotation.set(
					THREE.MathUtils.degToRad(this.currentStateInstance.angleX.getValue() as number),
					this.getLightsAngleY((settings.followOrientation.getValue() as number) === 1),
	...
	private getLightsAngleY(followOrientation: boolean) {
		...
		return THREE.MathUtils.degToRad(
			(this.currentStateInstance.angleY.getValue() as number) + (followOrientation ? orientationAngle : 0),
		);
	}

	private getMapObjectLayerDepthOffset(): number {
		const kind = this.currentStateInstance?.graphicKind;
		if (kind !== ELEMENT_MAP_KIND.FLOORS && kind !== ELEMENT_MAP_KIND.AUTOTILES) {
			return 0;
		}
		return (
			(this.positionLayer + (this.currentStateInstance.layer.getValue() as number)) *
	...
	private static getMapObjectLandCollision(
		position: THREE.Vector3,
	): { collision: StructMapElementCollision; layer: number; y: number } | null {
		...
		let result: { collision: StructMapElementCollision; layer: number; y: number } | null = null;
	...
	private static isPositionOnLand(position: THREE.Vector3, land: StructMapElementCollision): boolean {
		const landPosition = land.p.toVector3();
		const width = land.b?.[3] ?? 1;
		const height = land.b?.[4] ?? 1;
		return (
			position.x >= landPosition.x - 0.5 &&
	...
	static getTerrainAt(position: THREE.Vector3): number {
		...
		for (let y = squarePosition.y; y >= -Scene.Map.current.mapProperties.depth; y--) {
			...
			for (const land of lands) {
				if (
					MapObject.isPositionOnLand(position, land) &&
