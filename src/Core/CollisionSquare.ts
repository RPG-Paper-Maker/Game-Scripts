...
class CollisionSquare {
     ...
     *  @returns {number[][]}
     */
    static unionSquares(squares: number[][], l: number, w: number, h: number): 
        number[][] {
        ...
        let s: number[], a: number, b: number, c: boolean, tempW: number, tempH: 
            number, kk: number, m: number, temp: number, tempArray: number[];
        for (j = 0; j < h; j++) {
            k = j * w;
            for (i = 0; i < w; i++) {
                if (boolGrid[i + k]) {
                    ...
                    for (a = i + 1; a < w && tempW === -1; a++) {
                        ...
                        if (boolGrid[a + k]) {
                            if (squares[a + k - 1][0] + squares[a + k - 1][2] 
                                === Datas.Systems.SQUARE_SIZE && squares[a + k][
                                0] === 0)
                            {
