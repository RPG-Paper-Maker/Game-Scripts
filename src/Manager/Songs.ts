...
class Songs {
    ...
    public static isProgressionMusicEnd = true;
    public static isMusicNone = true;
    public static volumes: number[] = [];
    public static starts: number[] = [];
    public static ends: number[] = [];
    public static current: typeof Howl[] = [];
    public static currentSounds: typeof Howl[];
     ...
     */
    static playMusic(kind: SongKind, id: number, volume: number, start: number, 
        end: number)
    {
        if (id < 1) {
            switch (kind) {
     ...
     */
    static stopSong(kind: SongKind, time: number, seconds: number, id: number = -1, 
        pause: boolean = false): boolean
    {
        System.PlaySong.currentPlayingMusic = new System.PlaySong(SongKind.Music);
     ...
     */
    static playSound(id: number, volume: number) {
        if (id === -1) {
            return;
        }
     ...
     */
    static playMusicEffect(id: number, volume: number, currentState: Record<
        string, any>): boolean
    {
     ...
     *  @param {number} end - The end of the song
     */
    static initializeProgressionMusic(i: number, f: number, equation: number, 
        end: number)
    {
        this.progressionMusic = ProgressionTable.createFromNumbers(i, f, 
            equation);
